...
本文档根据样本文档 `D:\ziliao\毕业设计文字内容\计算机2201张毅(1).docx` 提取论文结构、章节写法、图表安排和行文要点，并结合当前 `D:\ziliao\dev` 项目的实际技术栈整理。后续写论文时，可以直接以本文档作为章节蓝图。

## 1. 样本文档结构提取结果

### 1.1 整体结构

样本文档属于“应用开发类毕业设计论文”，不是纯理论研究型论文。整体顺序如下：

1. 封面
2. 原创性声明
3. 中文题名、专业班级、学生姓名、指导教师
4. 中文摘要与关键词
5. 英文题名、英文摘要与关键词
6. 目录
7. 正文第 1 章到第 7 章
...
```text
为什么做 -> 用什么技术做 -> 系统需要什么 -> 系统怎么设计 -> 核心功能怎么实现 -> 怎么测试 -> 总结和展望
```

### 1.2 章节篇幅规律

样本文档各章大致篇幅如下，可作为后续写作比例参考：

| 部分 | 写作重点 | 篇幅占比建议 |
|---|---|---|
| 摘要、目录前内容 | 项目目标、技术路线、核心功能、测试结论 | 约 5% |
| 第 1 章 绪论 | 背景、痛点、研究现状、论文结构 | 约 12% |
| 第 2 章 相关技术基础 | 技术栈说明，避免写成教程 | 约 10% |
| 第 3 章 系统需求分析 | 可行性、角色、功能需求、非功能需求 | 约 14% |
| 第 4 章 系统总体设计 | 架构、模块、数据库、安全设计 | 约 14% |
| 第 5 章 系统详细设计与实现 | 核心功能、代码对应关系、页面效果 | 约 30% |
| 第 6 章 系统测试 | 测试环境、功能测试、安全测试、性能兼容性 | 约 12% |
| 第 7 章 总结与展望 | 工作总结、成果、不足、后续方向 | 约 8% |

第 5 章是全文重点，应该写得最具体，必须对应实际代码、页面、接口和数据表。

## 2. 当前 dev 项目基础信息

从当前项目目录初步识别，论文应围绕一个 DevSecOps 或 Web 安全审计平台展开。
...
|---|---|
| Web 框架 | Flask |
| 数据访问 | Flask-SQLAlchemy、SQLAlchemy、Flask-Migrate |
| 数据库 | MySQL，PyMySQL 驱动 |
| 认证授权 | Flask-JWT-Extended |
| 跨域与限流 | Flask-Cors、Flask-Limiter |
| 异步任务 | Celery、Redis |
| 安全扫描 | Bandit、Wapiti3，项目内还有 SAST、DAST、SCA 服务模块 |
| 数据校验 | Marshmallow、email-validator |
| 报告导出 | WeasyPrint |
...
```text
app/api       接口层，包括 auth、tasks、sast、dast、sca、audit、admin、metrics、ai
app/models    数据模型，包括 user、task、sast_result、dast_result、sca_result、audit_log 等
app/services  业务服务，包括 auth_service、task_service、sast_service、dast_service、sca_service、audit_service 等
app/workers   Celery 异步任务，包括 sast_tasks、dast_tasks、sca_tasks
app/utils     响应、异常、文件、安全校验等通用工具
```

前端页面包括：

```text
LoginView / RegisterView       登录注册
ScanCreateView                 新建扫描
TaskListView                   任务列表与工作台
TaskDetailView                 任务详情与报告查看
AdminDashboardView             管理看板
AuditLogView                   审计日志
```

## 3. 摘要写作模板

中文摘要应控制在 300 到 500 字左右。写法采用“四段合一”的逻辑：

1. 研究背景和问题：说明软件开发和上线过程中存在安全检测滞后、人工审计效率低、扫描结果分散、整改追踪困难等问题。
2. 系统方案和技术路线：说明系统采用前后端分离架构，后端基于 Flask、SQLAlchemy、JWT、Celery 和 Redis，前端基于 Vue 3、Vite、Axios 和 Tailwind CSS。
3. 核心功能：说明实现了用户认证、扫描任务创建、SAST/DAST/SCA 检测、任务状态跟踪、漏洞结果展示、报告导出、管理看板和审计日志等功能。
4. 测试和结论：说明通过功能测试、安全测试、性能与兼容性测试，系统能够完成安全扫描任务闭环，具备一定的实用价值。

关键词建议：

```text
DevSecOps；安全审计；静态代码分析；动态应用扫描；Flask；Vue
```

英文摘要不要逐字机翻中文摘要，应保持信息一致，句子简单清晰。
...
- 软件项目迭代速度提升后，安全测试如果只放在上线前，会导致问题发现晚、整改成本高。
- 人工安全审计存在效率低、结果分散、复测困难、过程不可追溯的问题。
- SAST、DAST、SCA 可以覆盖代码、运行时应用和第三方依赖三个维度，但需要统一平台进行任务管理和结果沉淀。
- 本系统的价值在于把“创建扫描任务、执行检测、查看风险、导出报告、审计操作”串成闭环。

国内外研究现状不要写成资料堆砌，应按方向归纳：

| 方向 | 可写内容 |
|---|---|
| 国外 | DevSecOps 理念、OWASP Top 10、SAST/DAST 工具链、CI/CD 安全集成、商业安全平台 |
| 国内 | 高校和企业对代码审计、漏洞扫描、依赖治理、安全管理平台的研究与实践 |
| 述评 | 现有工具能力强但结果分散，轻量化、可部署、适合课程设计和中小团队使用的平台仍有实践价值 |

### 5.2 第 2 章 相关技术基础

写作目标：说明为什么当前技术栈适合实现这个系统。
...
- 每个技术都要和项目用途绑定。
- Flask 写接口组织、蓝图、请求处理。
- SQLAlchemy 写数据模型、关系映射和查询。
- JWT 写前后端分离下的登录状态保持。
- Celery 和 Redis 写耗时扫描任务的异步执行。
- Vue、Vite、Axios 写前端页面、路由和接口交互。
- Bandit、Wapiti、SCA 写安全检测能力来源。

可采用句式：

```text
在本系统中，Flask 主要用于提供 RESTful 接口，用户认证、扫描任务、审计日志等功能均通过蓝图模块进行组织。与传统单体页面相比，该方式能够使后端接口与前端页面解耦，便于后续扩展 SAST、DAST 和 SCA 等不同类型的扫描能力。
```
...
| 角色 | 主要职责 | 典型功能 |
|---|---|---|
| 普通用户/开发人员 | 提交扫描任务并查看个人任务结果 | 注册登录、新建扫描、查看任务、查看报告、重新运行 |
| 安全测试人员 | 对代码或 Web 目标进行安全检测和结果分析 | SAST、DAST、SCA 扫描、风险详情查看、报告导出 |
| 管理员 | 管理平台用户、全局任务和审计信息 | 管理看板、用户管理、审计日志、全局统计 |

核心功能需求：

| 模块 | 功能点 | 写作说明 |
|---|---|---|
| 用户认证 | 注册、登录、Token 校验、退出登录 | 保证平台访问身份明确 |
| 扫描任务 | 新建任务、任务列表、任务详情、取消、重新运行 | 平台业务主线 |
| SAST | 上传代码或指定路径、调用 Bandit、解析问题 | 面向源代码缺陷 |
| DAST | 输入目标 URL、授权确认、调用 Wapiti、记录结果 | 面向运行中 Web 应用 |
| SCA | 识别依赖组件、记录依赖风险 | 面向第三方组件安全 |
| 报告 | 汇总风险、展示详情、导出 PDF | 支撑整改和归档 |
| 管理看板 | 全局任务数、风险等级、趋势或分布 | 支撑管理视角 |
| 审计日志 | 记录登录、任务创建、取消、重跑等操作 | 支撑追溯 |

非功能需求：

- 安全性：JWT 鉴权、密码保护、接口权限、扫描授权确认、上传文件限制。
- 可靠性：扫描任务异步执行，状态可跟踪，异常有错误信息。
- 易用性：统一入口创建扫描，任务状态清晰，结果按风险等级展示。
- 可维护性：后端按 api、models、services、workers 分层，前端按 views、api、router、store 组织。
- 可扩展性：后续可以扩展更多扫描工具、规则库、报告格式和通知方式。
...
```text
系统采用前后端分离架构。前端基于 Vue 3 构建登录、扫描创建、任务列表、任务详情、管理看板和审计日志等页面；后端基于 Flask 提供 RESTful 接口，并通过 SQLAlchemy 操作数据库。对于 SAST、DAST 和 SCA 等耗时任务，系统使用 Celery 将扫描过程放入异步任务队列执行，Redis 作为消息队列或任务中间件。扫描完成后，后端将任务状态、结果摘要和漏洞详情写入数据库，前端通过接口查询并展示。
```
...
```text
图4-1 系统运行流程图
用户登录 -> 创建扫描任务 -> 后端校验参数 -> Celery 执行扫描 -> 解析结果 -> 写入数据库 -> 前端展示报告
...
业务层 Service
任务层 Celery Worker
数据层 SQLAlchemy + MySQL + 文件系统
工具层 Bandit / Wapiti / SCA 检测

图4-3 数据库 E-R 图
users、tasks、sast_results、sast_issues、dast_results、dast_issues、sca_results、sca_issues、audit_logs、ai_issue_insights
```
...
| 数据表 | 主要字段 | 用途 |
|---|---|---|
| users | id、username、password_hash、email、role、status | 保存用户账号信息 |
| tasks | id、user_id、task_type、status、progress、target_path、target_url、scanner_engine、report_path | 保存扫描任务主信息 |
| sast_results | task_id、summary、score、started_at、finished_at | 保存静态扫描结果摘要 |
| sast_issues | task_id、severity、cwe、file_path、line、message | 保存 SAST 问题详情 |
| dast_results | task_id、target_url、summary、started_at、finished_at | 保存动态扫描结果摘要 |
| dast_issues | task_id、severity、url、parameter、description、solution | 保存 DAST 漏洞详情 |
| sca_results | task_id、summary、dependency_count | 保存依赖分析结果 |
| sca_issues | task_id、package_name、version、vulnerability_id、severity | 保存组件风险详情 |
| audit_logs | user_id、action、resource_type、resource_id、detail、ip、created_at | 保存操作审计记录 |

安全设计重点：

- 默认业务接口需要登录后访问。
- JWT 用于识别当前用户。
- DAST 扫描需要用户确认目标授权，避免对未授权目标发起扫描。
- 文件上传需要限制类型、大小和保存路径。
- 审计日志记录关键操作，便于追踪问题。

### 5.5 第 5 章 系统详细设计与实现

写作目标：结合实际代码说明核心功能如何落地。

本章要避免只写“系统实现了某功能”。应按“业务问题 -> 实现思路 -> 关键代码/模块 -> 页面效果 -> 设计价值”展开。
...
- 用户登录后由后端生成 JWT。
- 前端保存登录状态，路由守卫拦截未登录访问。
- 请求工具在后续接口请求中携带 Token。
- 后端使用 `jwt_required()` 保护任务、审计和管理接口。
...
- 任务类型包括 SAST、DAST、SCA。
- 任务状态包括 PENDING、RUNNING、SUCCESS、FAILED、TIMEOUT、CANCELLED。
- 任务列表支持按类型、状态和时间筛选，并使用分页返回。
- 取消和重新运行任务会写入审计日志。
- 重新运行任务时，根据任务类型重新投递到对应 Celery worker。
...
```text
backend/app/api/sast.py
backend/app/services/sast_service.py
backend/app/workers/sast_tasks.py
backend/app/models/sast_result.py
backend/app/models/sast_issue.py
```

写作重点：

- SAST 面向源代码或上传文件进行静态分析。
- 系统调用 Bandit 等工具执行扫描。
- 扫描结果被解析为结果摘要和问题明细。
- 问题明细可以包含风险等级、文件路径、代码行号、CWE 编号、说明和修复建议。
...
backend/app/workers/dast_tasks.py
backend/app/models/dast_result.py
backend/app/models/dast_issue.py
```

写作重点：

- DAST 面向运行中的 Web 应用目标 URL。
- 系统要求用户确认目标授权。
- 通过 Wapiti3 等工具进行动态检测。
- 结果按 URL、参数、漏洞类型、风险等级和修复建议保存。
...
```text
backend/app/api/sca.py
backend/app/services/sca_service.py
backend/app/workers/sca_tasks.py
backend/app/models/sca_result.py
backend/app/models/sca_issue.py
```

写作重点：

- SCA 用于识别项目依赖组件和潜在漏洞。
- 可围绕 `requirements.txt`、前端依赖或上传代码包展开。
- 论文中要强调它补充了 SAST 和 DAST 对第三方组件风险覆盖不足的问题。
...
- 报告应包含任务基本信息、扫描结论、风险统计、问题明细和修复建议。
- PDF 导出用于归档、提交整改和答辩展示。
...
- 审计日志记录关键业务操作。
- 日志字段应包含操作人、操作类型、资源类型、资源 ID、请求信息、结果和时间。
- 审计日志的价值是问题排查、责任追踪和安全管理。

### 5.6 第 6 章 系统测试

写作目标：证明系统能运行，核心流程能闭环。
...
|---|---|
| 操作系统 | Windows 本地开发环境 |
| 后端环境 | Python、Flask、Celery |
| 前端环境 | Node.js、npm、Vue 3、Vite |
| 数据库 | MySQL |
| 缓存/队列 | Redis |
| 浏览器 | Chrome 或 Edge |
| 测试账号 | 根据实际初始化数据填写 |
...
| 编号 | 测试内容 | 操作步骤 | 预期结果 |
|---|---|---|---|
| T01 | 用户注册 | 输入用户名、邮箱和密码提交 | 注册成功或返回明确校验提示 |
| T02 | 用户登录 | 输入正确账号密码登录 | 登录成功并进入工作台 |
| T03 | 未登录访问 | 直接访问任务列表接口 | 返回未认证提示 |
| T04 | 新建 SAST 任务 | 上传或选择代码目标并提交 | 任务创建成功，状态进入队列 |
| T05 | 新建 DAST 任务 | 输入 URL 并确认授权 | 任务创建成功并开始动态扫描 |
| T06 | 新建 SCA 任务 | 提交依赖分析目标 | 生成依赖检测任务 |
| T07 | 任务列表筛选 | 按任务类型和状态筛选 | 返回符合条件的数据 |
| T08 | 任务详情查看 | 打开扫描完成任务 | 显示结果摘要和问题明细 |
| T09 | 任务取消 | 对运行中任务执行取消 | 状态更新为取消或返回合理提示 |
| T10 | 重新运行 | 对历史任务执行重新运行 | 创建新的扫描任务 |
| T11 | 报告导出 | 点击导出 PDF | 生成报告文件 |
| T12 | 审计日志 | 完成任务创建或取消后查看日志 | 出现对应操作记录 |
...
- 任务列表分页查询是否稳定。
- 多次刷新任务详情和管理看板是否正常。
- 扫描任务异步执行时前端是否能看到状态变化。
- Chrome、Edge 等现代浏览器页面是否正常显示。

### 5.7 第 7 章 总结与展望

总结要写已经完成了什么：

- 完成了基于 Flask 和 Vue 的 DevSecOps 安全审计平台。
- 实现了用户认证、任务管理、SAST、DAST、SCA、报告导出、管理看板和审计日志。
- 通过异步任务机制解决扫描耗时问题。
- 通过统一结果展示和报告导出提高安全审计结果的可读性和可追溯性。

不足要写真实但不过度否定：

- 当前扫描工具和规则覆盖范围有限。
- DAST 扫描深度受目标授权范围、网络环境和测试数据影响。
- SCA 漏洞库和依赖识别能力仍可扩展。
- 自动化测试、持续集成和生产监控还不够完善。
- 多用户团队协作、项目空间、通知机制等能力可以继续增强。

展望可写：

- 接入更多扫描引擎和漏洞知识库。
- 增加 CI/CD 集成能力。
- 增加风险趋势分析、修复状态跟踪和复测闭环。
- 增加邮件、企业微信、钉钉等通知方式。
- 完善 Docker、Nginx、HTTPS、备份和监控部署方案。

## 6. 图表清单建议

样本文档大量使用表格和图来支撑论文，当前项目也应准备类似材料。
...
| 图号 | 名称 | 放置章节 | 获取方式 |
|---|---|---|---|
| 图4-1 | 系统运行流程图 | 4.1 | 用流程图绘制 |
| 图4-2 | 系统总体架构图 | 4.1 | 用架构图绘制 |
| 图4-3 | 功能模块结构图 | 4.2 | 根据模块绘制 |
| 图4-4 | 数据库 E-R 图 | 4.3 | 根据模型绘制 |
| 图5-1 | 用户登录与权限校验流程图 | 5.1 | 用流程图绘制 |
| 图5-2 | 新建扫描页面 | 5.2 | 截图 |
| 图5-3 | 任务列表页面 | 5.2 | 截图 |
| 图5-4 | SAST 扫描结果页面 | 5.3 | 截图 |
| 图5-5 | DAST 扫描结果页面 | 5.4 | 截图 |
| 图5-6 | 任务详情与报告页面 | 5.6 | 截图 |
| 图5-7 | 管理看板页面 | 5.7 | 截图 |
| 图5-8 | 审计日志页面 | 5.8 | 截图 |
| 图6-1 | 功能测试结果示意图 | 6.2 | 拼图或截图 |

## 7. 附录接口清单模板

附录 A 可以整理主要接口，不需要把所有内部接口都写得过细。建议格式：
...
| 任务 | `/api/tasks/{id}` | GET | 查询任务详情 |
| 任务 | `/api/tasks/{id}/cancel` | POST | 取消任务 |
| 任务 | `/api/tasks/{id}/rerun` | POST | 重新运行任务 |
| SAST | `/api/sast/tasks` | POST | 创建静态扫描任务 |
| DAST | `/api/dast/tasks` | POST | 创建动态扫描任务 |
| SCA | `/api/sca/tasks` | POST | 创建依赖分析任务 |
| 审计 | `/api/audit/logs` | GET | 查询审计日志 |
| 管理 | `/api/admin/dashboard` | GET | 查询管理看板数据 |
| 指标 | `/api/metrics` | GET | 查询统计指标 |

正式写作前应以实际代码中的路由为准核对接口路径。

## 8. 参考文献方向

参考文献数量建议 15 到 20 条。样本文档使用了官方文档、教材、标准和领域资料混合的方式。当前项目可选方向：
...
- Bandit 官方文档或项目文档
- Wapiti 官方文档
- 软件工程教材，如 Pressman、Sommerville
- DevSecOps、安全测试、软件供应链安全相关中文论文或标准
...
- 以工程实现为主，不写过多理论推导。
- 每一章都要和系统实际功能对应。
- 允许使用较自然的说明性语言，但要保持论文语气。
- 多使用“本文”“本系统”“该模块”“通过这种方式”等表述。
- 避免口语化词语，如“挺好”“凑合”“一堆”等，正式论文中应改为“较好”“基本满足”“大量”。

推荐句式：

```text
本系统采用……实现……，其主要作用是……
在该模块中，前端负责……，后端负责……，数据库用于保存……
通过上述设计，系统能够……
测试结果表明，系统在……方面达到了预期要求。
```
...
```text
该功能流程相对清晰，主要包括参数提交、后端校验、任务创建和结果返回四个步骤。
受开发周期和测试环境限制，当前系统在……方面仍有进一步完善空间。
```

## 10. 后续写论文时的执行顺序

建议后续按以下顺序写，而不是从第 1 章硬写到第 7 章：

1. 先确认论文题名和系统定位。
2. 梳理实际代码中的功能模块、接口、数据表和页面截图。
3. 先写第 4 章和第 5 章，因为这两章最依赖项目实际情况。
4. 再写第 3 章需求分析，让需求与已经实现的功能一致。
5. 写第 6 章测试，按实际运行结果整理测试表。
6. 写第 2 章技术基础，只保留和项目相关的技术。
7. 写第 1 章绪论、摘要、总结与展望。
8. 最后补参考文献、致谢、附录接口清单和目录。

核心原则：论文内容必须和 `D:\ziliao\dev` 项目的实际代码、页面、接口、数据库模型保持一致。后续正式写作前，应再次扫描项目代码，核对模块名称、接口路径、数据表字段和页面截图。

## 11. 分轮写作流程与提示词

不要让 AI 一口气写完整篇论文。毕业论文需要和项目代码、页面、接口、数据库模型保持一致，一次性生成容易出现技术栈写错、功能编造、章节前后不一致、测试无法对应实现等问题。建议按下面轮次推进，每一轮完成后先检查，再进入下一轮。

### 11.1 总提示词

后续找新的 AI 写论文时，先发送下面这段作为总要求：

```text
你是毕业论文写作助手。请先阅读 D:\ziliao\dev\paper_writing_outline.md 和 D:\ziliao\dev 项目代码，严格按照提纲和项目实际功能撰写论文。论文主题是基于当前 dev 项目的系统设计与实现。不要一口气写完整篇论文，请按轮次逐步完成。每一轮写作前都要核对项目实际代码、接口、页面和数据模型；每一轮完成后都要检查是否存在编造功能、技术栈不一致、接口路径不准确、章节前后不一致的问题。论文语言要求正式、连贯、符合本科毕业设计论文风格。
```

### 11.2 第 0 轮：项目事实梳理

目标：先让 AI 理清项目事实，不写正文。

提示词：

```text
第 0 轮：请阅读 D:\ziliao\dev\paper_writing_outline.md 和 D:\ziliao\dev 项目代码，先不要写论文正文。请输出以下内容：
1. 推荐论文题目。
2. 系统定位和主要业务目标。
3. 后端技术栈、前端技术栈、数据库和异步任务相关技术。
4. 后端主要目录、接口模块、服务模块、数据模型。
5. 前端主要页面、路由和接口调用模块。
6. 系统核心功能模块清单。
7. 主要数据表或模型清单。
8. 后续写论文时必须避免编造的内容。

要求：所有结论必须来自 paper_writing_outline.md 或项目实际代码；不确定的地方标注“需进一步核对”，不要猜。
```

本轮检查项：

- 是否把项目写成 Java、Spring Boot、MyBatis 项目。
- 是否准确识别 Flask、SQLAlchemy、JWT、Celery、Redis、Vue、Vite。
- 是否确认了 SAST、DAST、SCA、任务管理、审计日志等核心模块。
- 是否没有开始写正式章节正文。

### 11.3 第 1 轮：第 4 章系统总体设计

目标：先写总体设计，因为它最依赖项目结构，也会约束后续章节。

提示词：

```text
第 1 轮：请根据第 0 轮梳理结果和 D:\ziliao\dev\paper_writing_outline.md，撰写论文第 4 章“系统总体设计”。
...
要求：
1. 必须结合当前项目的 Flask、Vue、SQLAlchemy、Celery、Redis、SAST、DAST、SCA 实现。
2. 数据库设计必须参考项目 models 目录，不要编造不存在的数据表。
3. 可以用“图4-1 系统运行流程图”“图4-2 系统总体架构图”“表4-1 主要数据表设计”等占位说明，但不要生成虚假的截图。
4. 写作语言要像正式本科毕业论文，避免口语化。
5. 章节内部要体现前后端、任务队列、扫描工具、数据库之间的数据流转关系。
```

本轮检查项：

- 架构是否符合项目实际。
- 是否解释了异步扫描任务为什么需要 Celery 和 Redis。
- 数据库表或模型名称是否和代码一致。
- 安全设计是否包含 JWT、扫描授权、上传限制和审计日志。

### 11.4 第 2 轮：第 5 章系统详细设计与实现

目标：写全文最重要章节，必须和代码、接口、页面强绑定。

提示词：

```text
第 2 轮：请根据 D:\ziliao\dev 项目代码和已完成的第 4 章，撰写第 5 章“系统详细设计与实现”。
...
要求：
1. 每个小节都要对应实际代码目录或页面，例如 backend/app/api、backend/app/services、backend/app/models、backend/app/workers、frontend/src/views。
2. 不要大段粘贴代码，重点解释业务流程、实现思路、关键类或模块的作用。
3. 任务状态、任务类型、扫描结果、审计日志等内容必须与项目代码一致。
4. 可以插入图表占位，例如“图5-2 新建扫描页面”“图5-3 任务详情页面”，但不要编造截图内容。
5. 语言要求正式、具体，避免只写“实现了增删改查”。
```

本轮检查项：

- 是否覆盖登录、任务、SAST、DAST、SCA、报告、看板、审计。
- 是否出现项目没有的功能或页面。
- 是否把实现写得过于空泛。
- 是否和第 4 章模块划分一致。

### 11.5 第 3 轮：第 3 章系统需求分析

目标：让需求与第 4、5 章已经写出的设计和实现保持一致。

提示词：

```text
第 3 轮：请根据已完成的第 4 章和第 5 章，撰写第 3 章“系统需求分析”。
...
要求：
1. 需求必须能对应第 4、5 章已经写到的实际功能。
2. 用户角色建议包括普通用户或开发人员、安全测试人员、管理员；如代码中角色设计不同，以代码为准。
3. 功能需求重点写用户认证、扫描任务、SAST、DAST、SCA、报告、管理看板、审计日志。
4. 非功能需求重点写安全性、可靠性、易用性、可维护性、可扩展性。
5. 不要写当前系统没有实现且第 5 章没有对应内容的需求。
```
...
### 11.6 第 4 轮：第 6 章系统测试

目标：用测试证明核心功能闭环，不写无法验证的测试。

提示词：

```text
第 4 轮：请根据项目实际功能和已完成的第 3、4、5 章，撰写第 6 章“系统测试”。
...
要求：
1. 测试用例要覆盖登录注册、创建扫描任务、SAST、DAST、SCA、任务列表、任务详情、取消、重新运行、报告导出、审计日志。
2. 测试环境要符合当前项目，例如 Python、Flask、MySQL、Redis、Celery、Node.js、Vue、Vite、浏览器。
3. 安全测试重点写 JWT、未登录访问、越权访问、DAST 授权确认、文件上传限制。
4. 不要编造大规模压测数据。性能测试可以定位为本地环境下的分页查询、任务状态刷新和常见页面访问验证。
5. 测试结果表述要稳妥，使用“基本满足”“达到毕业设计预期”等表达。
```
...
### 11.7 第 5 轮：第 2 章相关技术基础

目标：只写项目中真实使用的技术，不写成教程。

提示词：

```text
第 5 轮：请根据项目实际技术栈，撰写第 2 章“相关技术基础”。
...
要求：
1. 每种技术都要说明它在本系统中的作用。
2. 不要写与项目无关的技术，例如 Spring Boot、MyBatis、Element Plus、ECharts，除非代码中实际使用。
3. 技术介绍要服务于后续系统设计和实现，不要写成百科。
4. 可以在文中加入参考文献编号占位，但后续需要统一整理。
```
...
目标：在已经明确系统实现后，再写背景和研究意义，避免空泛。

提示词：

```text
第 6 轮：请根据已完成的第 2、3、4、5、6 章，撰写第 1 章“绪论”。
...
要求：
1. 背景要围绕软件安全、DevSecOps、SAST、DAST、SCA、审计追踪和报告归档展开。
2. 国内外现状按方向归纳，不要堆砌资料。
3. 研究内容必须和后文章节实际功能一致。
4. 论文结构说明要准确对应全文 7 章。
5. 语言正式，不要出现口语化表达。
...
```text
第 7 轮：请根据已完成的第 1 到第 6 章，撰写中文摘要、关键词、英文摘要、英文关键词、第 7 章“总结与展望”、参考文献、致谢和附录 A 主要接口清单。

要求：
1. 中文摘要包括研究背景、系统方案、核心功能、测试结论，控制在 300 到 500 字左右。
2. 英文摘要与中文摘要信息一致，语言清晰，不要直译得过于生硬。
3. 关键词建议包含 DevSecOps、安全审计、静态代码分析、动态应用扫描、Flask、Vue。
4. 第 7 章包括工作总结、主要成果、当前不足、后续展望。
5. 参考文献优先包含 Flask、SQLAlchemy、Vue、Vite、JWT RFC、Celery、Redis、OWASP Top 10、Bandit、Wapiti、软件工程教材等。
6. 附录 A 的接口清单必须以项目实际路由为准，不能直接沿用模板路径。
7. 致谢语言简洁正式，不要过度夸张。
```
...
```text
第 8 轮：请对整篇论文进行一致性检查和润色。重点检查：
1. 论文题目、系统名称、技术栈是否全文一致。
2. 章节编号、图编号、表编号是否连续。
3. 第 3 章需求、第 4 章设计、第 5 章实现、第 6 章测试是否互相对应。
4. 是否存在项目没有实现的功能、接口、页面或数据表。
5. 是否误写为 Spring Boot、MyBatis、Element Plus 等非当前项目技术。
6. 文中引用编号和参考文献是否一致。
7. 摘要、总结和正文是否表述一致。
8. 是否存在口语化表达、重复段落、明显 AI 味表达。
...
本节用于保存后续按轮次生成的论文材料。写作时应以本节内容为阶段性草稿，并在进入下一轮前核对项目代码、接口、页面和数据模型，避免出现技术栈、功能模块和接口路径不一致的问题。
...
当前 `D:\ziliao\dev` 项目是一个面向 Web 应用和代码项目的 DevSecOps 安全审计平台。系统围绕“创建扫描任务、异步执行检测、保存扫描结果、展示风险详情、导出报告、记录审计日志”形成安全检测闭环，主要解决安全检测任务分散、结果追踪困难、人工归档效率低和操作过程缺少审计记录等问题。

系统主要业务目标包括：

1. 为用户提供统一的安全扫描入口。
2. 支持 SAST、DAST、SCA 三类安全检测任务。
3. 通过任务状态、风险结果和报告导出帮助用户跟踪扫描过程。
4. 通过管理员看板和审计日志支持全局统计与操作追溯。
...
| 异步任务 | Celery |
| 队列与缓存 | Redis |
| 安全扫描 | Bandit、Wapiti、pip-audit，配置中还支持 Semgrep |
| 报告导出 | HTML、PDF，PDF 依赖 WeasyPrint |
| 数据校验 | email-validator、自定义 validators |
| 数据库 | 代码默认使用 `sqlite:///devsecops.db`，可通过 `DATABASE_URL` 切换；`requirements.txt` 包含 PyMySQL，若论文写 MySQL 需结合实际部署环境进一步核对 |
...
#### 12.1.4 后端目录、接口模块、服务模块和数据模型

后端主要目录包括：

| 目录 | 作用 |
|---|---|
| `backend/app/api` | 接口层，按业务模块注册 Flask Blueprint |
| `backend/app/services` | 业务服务层，封装认证、任务、扫描、审计和报告导出逻辑 |
| `backend/app/models` | 数据模型层，定义用户、任务、扫描结果、审计日志等模型 |
| `backend/app/workers` | Celery 异步任务层，执行 SAST、DAST、SCA 扫描 |
| `backend/app/utils` | 通用工具层，包括响应、异常、权限、文件处理、参数校验和 CWE 映射 |
| `backend/uploads` | 上传文件保存目录 |
| `backend/reports` | 扫描报告输出目录 |
...
| `tasks.py` | `/api/tasks` | 任务列表、取消任务、重新运行任务 |
| `sast.py` | `/api/sast` | 创建 SAST 任务、查看结果、下载或导出报告 |
| `dast.py` | `/api/dast` | 创建 DAST 任务、查看结果、下载或导出报告 |
| `sca.py` | `/api/sca` | 创建 SCA 任务、查看结果、下载或导出报告 |
| `audit.py` | `/api/audit-logs` | 审计日志查询、筛选和统计 |
| `admin.py` | `/api/admin` | 管理员看板、全局任务查询 |
| `metrics.py` | `/api/metrics` | 任务统计指标 |
| `ai.py` | `/api/ai` | 单个漏洞的辅助解释与修复建议 |
| `health.py` | `/api/health` | 健康检查 |

服务模块包括 `auth_service.py`、`task_service.py`、`sast_service.py`、`dast_service.py`、`sca_service.py`、`report_export_service.py`、`audit_service.py` 和 `ai_service.py`。这些服务分别对应用户认证、任务生命周期管理、三类扫描结果解析、报告导出、审计日志处理和 AI 辅助分析。

数据模型包括 `User`、`Task`、`SastResult`、`SastIssue`、`DastResult`、`DastIssue`、`ScaResult`、`ScaIssue`、`AuditLog` 和 `AiIssueInsight`。任务类型包括 `SAST`、`DAST`、`SCA`，任务状态包括 `PENDING`、`RUNNING`、`SUCCESS`、`FAILED`、`TIMEOUT`、`CANCELLED`。

#### 12.1.5 前端页面、路由和接口调用模块
...
| `/scan/new` | `ScanCreateView.vue` | 统一新建扫描入口 |
| `/sast` | 重定向到 `/scan/new?type=sast` | SAST 扫描入口 |
| `/dast` | 重定向到 `/scan/new?type=dast` | DAST 扫描入口 |
| `/tasks` | `TaskListView.vue` | 任务列表与工作台 |
| `/tasks/:id` | `TaskDetailView.vue` | 任务详情与报告查看 |
| `/admin` | `AdminDashboardView.vue` | 管理员看板 |
| `/audit-logs` | `AuditLogView.vue` | 审计日志 |

前端接口调用模块包括：

| 文件 | 作用 |
|---|---|
| `frontend/src/api/axios.ts` | Axios 实例，统一设置 `baseURL`，自动携带 Bearer Token |
| `frontend/src/api/auth.ts` | 注册、登录、获取当前用户 |
| `frontend/src/api/task.ts` | 任务列表、扫描任务创建、任务详情、取消、重跑、报告下载和导出 |
| `frontend/src/api/admin.ts` | 管理员看板和全局任务查询 |
| `frontend/src/api/audit.ts` | 审计日志查询 |
| `frontend/src/store/auth.ts` | Token 保存、读取、清除和登录状态判断 |
...
3. 扫描任务列表、筛选和分页。
4. 扫描任务取消与重新运行。
5. SAST 静态扫描：上传 `.py`、`.zip`、`.tar.gz` 等文件，调用 Bandit 或 Semgrep。
6. DAST 动态扫描：提交目标 URL，必须确认授权，调用 Wapiti。
7. SCA 依赖扫描：上传 `.txt` 或 `.zip`，定位 `requirements.txt`，调用 pip-audit。
8. 任务详情与风险结果展示。
9. JSON 报告下载。
10. HTML 和 PDF 报告导出。
11. 管理员看板：任务统计、风险摘要、运行状态、最近审计日志和失败任务等。
12. 审计日志：记录登录、注册、任务创建、取消、重跑、报告查看、报告导出和 DAST 授权等操作。
13. AI 漏洞解释辅助：`/api/ai/issue-insight`，可作为辅助功能描述，但不应扩写为完整智能审计系统。

#### 12.1.7 主要数据表或模型清单

| 数据表/模型 | 主要字段或内容 | 用途 |
|---|---|---|
| `users` / `User` | `username`、`email`、`password_hash`、`role`、`is_active` | 保存用户账号、角色和启用状态 |
| `tasks` / `Task` | `task_type`、`status`、`progress`、`target_path`、`target_url`、`scanner_engine`、`report_path`、`queue_task_id` | 保存扫描任务主信息 |
| `sast_results` / `SastResult` | `issue_count`、`severity_distribution`、`confidence_distribution`、`file_distribution` | 保存 SAST 结果摘要 |
| `sast_issues` / `SastIssue` | `test_id`、`issue_severity`、`filename`、`line_number`、`cwe`、`owasp` | 保存 SAST 问题详情 |
| `dast_results` / `DastResult` | `target_url`、`issue_count`、`anomaly_count`、`crawled_pages`、`scan_scope` | 保存 DAST 结果摘要 |
| `dast_issues` / `DastIssue` | `category`、`level`、`method`、`path`、`parameter`、`http_request`、`cwe`、`owasp` | 保存 DAST 漏洞详情 |
| `sca_results` / `ScaResult` | `dependency_count`、`vulnerability_count`、`fixable_count`、`package_distribution` | 保存依赖扫描结果摘要 |
| `sca_issues` / `ScaIssue` | `package_name`、`installed_version`、`vulnerability_id`、`fix_versions`、`severity` | 保存组件漏洞详情 |
| `audit_logs` / `AuditLog` | `user_id`、`action`、`resource_type`、`resource_id`、`ip_address`、`user_agent`、`detail` | 保存关键操作审计记录 |
| `ai_issue_insights` / `AiIssueInsight` | `task_id`、`issue_id`、`meaning`、`impact`、`fix`、`verify` | 保存 AI 辅助分析缓存 |

#### 12.1.8 后续写作必须避免编造的内容

1. 不要写成 Spring Boot、MyBatis、Element Plus 或 ECharts 项目。
2. 不要写“管理员用户管理增删改查”，当前代码主要支持管理员看板和全局任务查询，未看到完整用户管理接口。
3. 不要把 DAST 写成可任意扫描公网目标。代码要求授权确认，并通过配置限制目标范围。
4. 不要把 SCA 写成通用全语言依赖扫描。当前实现主要围绕 Python `requirements.txt` 和 pip-audit。
5. 不要默认写数据库一定是 MySQL。代码默认 SQLite，支持通过环境变量配置数据库，若写 MySQL 需要进一步核对实际部署环境。
6. 不要将 CI/CD 自动流水线写成系统核心功能，除非后续专门核对脚本和文档后确认。
7. 不要把 AI 模块夸大为自动修复或完整智能审计，只能写为漏洞解释和修复建议辅助。
8. 不要把未挂载的前端页面当作当前主要路由页面，正式写作应以 `frontend/src/router/index.ts` 为准。

### 12.2 第 1 轮：第 4 章系统总体设计

以下内容为论文第 4 章草稿，已根据第 0 轮事实梳理和当前项目代码进行约束。后续第 5 章详细实现应继续与 `backend/app/api`、`backend/app/services`、`backend/app/models`、`backend/app/workers` 和 `frontend/src/views` 保持一致。
...
```text
用户注册/登录 -> 获取 JWT -> 创建扫描任务 -> 后端参数校验
-> 写入任务记录 -> 投递 Celery 任务 -> 执行 SAST/DAST/SCA 扫描
-> 解析扫描结果 -> 写入结果表和问题表 -> 前端查看详情/导出报告
-> 记录审计日志
```
...
Flask API 接口层
  -> Service 业务层
  -> Celery Worker 任务层
扫描工具 Bandit / Semgrep / Wapiti / pip-audit
  -> SQLAlchemy 数据模型
  -> 数据库、uploads、reports
```
...
图 4-4 可绘制为数据库 E-R 图，核心关系如下：

```text
users 1 --- n tasks
tasks 1 --- 1 sast_results
tasks 1 --- n sast_issues
tasks 1 --- 1 dast_results
tasks 1 --- n dast_issues
tasks 1 --- 1 sca_results
tasks 1 --- n sca_issues
users 1 --- n audit_logs
tasks 1 --- n ai_issue_insights
```
...
### 12.3 第 2 轮：第 5 章系统详细设计与实现

以下内容为论文第 5 章草稿，已根据当前项目代码中的接口、服务、模型、异步任务和前端页面进行核对。本章重点说明系统核心功能如何落地，避免直接堆砌代码。
...
### 12.4 第 3 轮：第 3 章系统需求分析

以下内容为论文第 3 章草稿。本轮参考了任务书《基于 DevSecOps 的自动化代码审计与漏洞扫描平台设计与实现》和开题报告中的研究目标、任务要求与研究内容，同时以当前项目实际代码和已完成的第 4、5 章为准。由于当前项目已在原 SAST/DAST 基础上扩展 SCA、HTML/PDF 报告导出、管理看板和更完整的审计日志，本章需求以最终项目功能为依据。
...
### 12.5 第 4 轮：第 6 章系统测试

以下内容为论文第 6 章草稿。本轮依据当前项目中的 `backend/tests` 自动化测试、`scripts` 实验脚本、`start_demo_env.ps1` 演示环境脚本以及现有联调记录整理。正式论文定稿前，需根据最终运行结果补充截图、测试数据表和实验图表。本章中的【需补图】、【需补表】、【需补实测数据】为后续材料准备标记。
...
【需补图 6-2：后端健康检查结果截图】建议展示 `/api/health` 返回的 `service`、`database`、`redis`、`dast_scanner` 和 `celery_mode`。

【需补图 6-3：前端工作台首页截图】建议展示任务统计卡、任务列表和新建扫描入口。
...
【需补图 6-5：统一新建扫描页面截图，需包含 SAST/SCA/DAST 切换入口】

【需补图 6-6：任务详情页截图，建议分别准备 SAST、DAST、SCA 三张结果图】
...
【需补图 6-10：DAST 授权确认表单截图】

【需补图 6-11：DAST 授权信息在任务详情或管理看板中的展示截图】
...
【需补图 6-12：有限并发探测结果截图或生成的 Markdown/CSV 摘要截图】

【需补图 6-13：任务统计卡片截图】

【需补图 6-14：PDF 报告导出成功截图】

【需补图表 6-1：DAST 靶场样本扫描结果柱状图，可展示不同靶场的问题数量或耗时】
...
【需补图 6-15：Chrome 页面展示截图】

【需补图 6-16：Edge 页面展示截图】
...
### 12.6 第 5 轮：第 2 章相关技术基础

以下内容为论文第 2 章草稿。本轮根据当前项目实际技术栈撰写，重点说明相关技术在本系统中的作用，不展开与项目无关的框架或工具。当前系统后端基于 Flask、Flask-JWT-Extended、Flask-SQLAlchemy、Celery、Redis 等实现；前端基于 Vue 3、Vite、TypeScript、Vue Router 和 Axios 实现；安全检测能力围绕 SAST、DAST 和 SCA 三类任务组织，分别对接 Bandit/Semgrep、Wapiti 和 pip-audit 等工具或命令配置。
...
以下内容为论文第 1 章草稿。本轮在已完成第 2、3、4、5、6 章的基础上撰写，研究背景、研究现状和研究内容均围绕当前项目实际功能展开。文中参考文献编号为占位形式，后续第 7 轮需要统一整理参考文献条目。
...
### 12.8 第 7 轮：摘要、总结、参考文献、致谢和附录

以下内容为论文首尾部分和附录草稿。本轮根据已完成的第 1 章至第 6 章撰写，并以当前项目实际路由、技术栈和功能边界为准。后续第 8 轮需要对全文引用编号、图表编号、术语和占位测试材料进行统一检查。
...
### 12.9 第 8 轮：全文一致性检查与润色

以下内容为第 8 轮全文一致性检查结果。本轮重点核对第 12 节中已经生成的阶段性论文草稿，包括摘要、第 1 章至第 7 章、参考文献、致谢和附录 A。检查范围覆盖论文题目、系统名称、技术栈、功能模块、接口路径、数据模型、角色权限、图表占位、引用编号和论文表述边界。

#### 12.9.1 发现的问题清单

1. 角色名称存在大小写表述差异。第 3 章和第 4 章曾使用 `USER`、`ADMIN` 描述权限角色，第 2 章使用 `user`、`admin` 描述接口返回值。代码中实际枚举项为 `UserRole.USER` 和 `UserRole.ADMIN`，返回值为 `user` 和 `admin`。该问题已在第 3、4 章和第 2 章关键段落中统一说明。

2. 接口路径需要以实际 Blueprint 为准。模板中曾建议 `/api/audit/logs` 和 `/api/tasks/{id}` 等路径，但当前项目实际审计日志接口为 `/api/audit-logs`，任务详情分别位于 `/api/sast/tasks/{task_id}`、`/api/dast/tasks/{task_id}` 和 `/api/sca/tasks/{task_id}`。第 7 轮附录 A 已按实际代码修正，不应再使用模板路径。

3. 管理员功能边界需要保持克制。当前代码支持管理员看板 `/api/admin/dashboard` 和全局任务查询 `/api/admin/tasks`，没有完整的管理员用户增删改查接口。因此全文应继续使用“管理看板、全局任务查询、风险统计、审计追踪”等表述，不应扩写为“用户管理系统”或“管理员用户管理增删改查”。

4. 数据库表述需要保持一致。项目默认数据库为 SQLite，测试环境使用内存 SQLite，可通过 `DATABASE_URL` 切换数据库，依赖中包含 PyMySQL。论文中不应固定写成 MySQL 系统；如果最终部署使用 MySQL，需要在测试环境和部署说明中补充实测配置。

5. SCA 能力边界需要保持一致。当前 SCA 主要面向 Python `requirements.txt` 和包含该文件的 ZIP 包，使用 pip-audit 检测依赖风险。第 7 章展望中可以写未来支持 `package.json`、`pom.xml`、`go.mod`，但正文实现部分不能写成已经支持多语言全生态依赖扫描。

6. DAST 能力边界需要保持一致。当前系统强调授权目标和本地/私有网络测试，默认不允许公网目标。全文已多处说明“不应表述为任意公网扫描平台”，后续定稿仍需保留该限制，避免安全合规表述过度。

7. 性能能力不能夸大。第 6 章定位为本地有限并发、分页查询、报告导出和真实异步链路验证，不是生产级压测。第 7 章“不足”部分已经明确“不应宣称具备生产级高并发能力”，后续若补充测试数据，也应使用“有限并发验证”“本地演示环境”等措辞。

8. 图表和测试数据仍有占位项。第 6 章保留了大量 `【需补图】`、`【需补表】`、`【需补实测数据】` 和 `【需补图表】` 标记，包括测试环境截图、pytest 输出、前端 build 输出、扫描结果截图、并发探测数据和兼容性测试截图。正式论文定稿前必须用最终运行材料替换。

9. 图表编号需要最终统一。当前第 6 章图编号从图 6-1 到图 6-16，表编号从表 6-1 到表 6-8，另有 `图表 6-1` 用于 DAST 靶场扫描结果柱状图。定稿时建议将“图表 6-1”改为正式图号或表号，例如“图 6-17 DAST 靶场样本扫描结果柱状图”，避免与图、表体系混用。

10. 参考文献编号与正文引用基本对应。正文第 1 章引用了 `[1]` 至 `[4]`，第 7 轮参考文献中已有对应条目；其他技术文献 `[5]` 至 `[16]` 可服务于第 2 章技术基础。后续定稿时可在第 2 章适当增加引用标注，例如 Flask、SQLAlchemy、Vue、Vite、JWT、Celery、Redis 等技术首次出现处。

11. 摘要、总结和正文总体一致。摘要中列出的用户认证、SAST/DAST/SCA、任务状态跟踪、报告导出、管理看板和审计日志均在第 3、4、5、6 章中有对应内容。需要注意摘要中的“系统测试结果表明”最终应依赖第 6 章实测数据，当前仍应在定稿阶段替换测试占位材料。

12. 论文题目和系统名称基本一致。当前推荐题名为“基于 Flask 和 Vue 的 DevSecOps 安全审计平台设计与实现”，正文中主要使用“DevSecOps 安全审计平台”“安全审计平台”等名称。后续定稿时建议统一封面、摘要、绪论和总结中的题名，不再混用“自动化代码审计与漏洞扫描平台”作为正式题名；该名称可以作为任务书背景或研究对象说明出现。

#### 12.9.2 建议修改方案

1. 定稿前先替换第 6 章全部测试占位项。建议按以下顺序准备材料：后端 pytest 输出、前端 build 输出、健康检查截图、登录与工作台截图、SAST/DAST/SCA 任务详情截图、报告导出截图、管理员看板截图、审计日志截图、有限并发脚本输出、浏览器兼容性截图。

2. 将第 6 章中的 `图表 6-1` 统一改为正式图编号。若最终以柱状图呈现 DAST 靶场样本扫描结果，建议编号为“图 6-17”；若以数据表呈现，则改为“表 6-9”。不要在正式论文中保留“图表”混合编号。

3. 统一角色描述。正文中技术实现部分使用 `UserRole.USER`、`UserRole.ADMIN`，接口返回值说明使用 `user`、`admin`，面向读者解释时使用“普通用户”“管理员”。安全测试人员可以作为使用场景描述，但不要写成代码中的第三种权限角色。

4. 统一接口路径格式。正文中如果使用 Flask 路由写法，可以保留 `<task_id>`；附录和论文正文面向读者时建议统一为 `{task_id}`。需要重点保持 `/api/audit-logs`、`/api/admin/dashboard`、`/api/admin/tasks`、`/api/metrics/task-summary` 与代码一致。

5. 统一数据库表述。第 2、3、4、6 章均应写为“默认 SQLite，可通过 `DATABASE_URL` 配置切换数据库”。只有在最终运行环境确实使用 MySQL 时，才在测试环境表中补充 MySQL 版本和连接方式。

6. 统一扫描能力边界。实现章节写“当前支持”；展望章节写“后续可扩展”。例如当前支持 Python 依赖 SCA，后续可扩展 Node.js、Java、Go；当前 DAST 面向授权目标，后续可完善扫描模板和漏洞去重。

7. 参考文献建议在第 2 章增加引用标注。示例：Flask 技术介绍后标注 `[5]`，SQLAlchemy 后标注 `[6]`，Vue 3 后标注 `[7]`，Vite 后标注 `[8]`，JWT 后标注 `[9]`，Celery 和 Redis 后分别标注 `[10]`、`[11]`，Bandit、Wapiti、pip-audit 后分别标注 `[12]`、`[13]`、`[14]`。

8. 删除或改写可能被误解为已实现的展望内容。第 7 章中“趋势图、负责人、整改状态、CI/CD 接入”等内容应明确放在后续展望中，不能移动到系统实现或测试结论部分。

#### 12.9.3 修改后的关键段落或统一文本
...
本文设计并实现的系统是一个面向毕业设计和中小型安全检测场景的轻量化 DevSecOps 安全审计平台。系统围绕 SAST、DAST 和 SCA 三类扫描任务，提供用户认证、任务创建、异步执行、状态跟踪、风险展示、报告导出、管理看板和审计日志等功能。系统不定位为生产级高并发商业平台，也不提供任意公网目标扫描能力。

**统一技术栈表述：**

系统采用前后端分离架构。后端基于 Flask 提供 RESTful API，使用 Flask-JWT-Extended 实现 JWT 认证，使用 Flask-SQLAlchemy 和 SQLAlchemy 进行数据访问，使用 Celery 和 Redis 执行异步扫描任务；前端基于 Vue 3、Vite、TypeScript、Vue Router、Axios 和 Tailwind CSS 构建单页应用。安全检测工具包括 Bandit、Semgrep、Wapiti 和 pip-audit，报告导出功能可使用 WeasyPrint 生成 PDF。

**统一数据库表述：**

项目代码默认使用 SQLite 作为本地开发数据库，测试环境使用内存 SQLite，并通过 `DATABASE_URL` 保留数据库切换能力。论文中除非补充最终部署实测材料，否则不应将系统固定描述为 MySQL 项目。

**统一角色权限表述：**

系统当前代码角色枚举项为 `UserRole.USER` 和 `UserRole.ADMIN`，接口返回值分别为 `user` 和 `admin`。普通用户和安全测试人员在权限上均属于普通用户范围，只能创建、查看、取消和重跑自己的任务；管理员可以访问管理看板、全平台任务列表和更完整的审计日志。系统未实现完整管理员用户增删改查功能。

**统一接口路径表述：**

认证接口包括 `/api/auth/register`、`/api/auth/login` 和 `/api/auth/me`；任务列表、取消和重跑接口分别为 `/api/tasks`、`/api/tasks/{task_id}/cancel` 和 `/api/tasks/{task_id}/rerun`；三类扫描任务分别使用 `/api/sast/tasks`、`/api/dast/tasks` 和 `/api/sca/tasks` 创建，并通过对应的 `/api/sast/tasks/{task_id}`、`/api/dast/tasks/{task_id}`、`/api/sca/tasks/{task_id}` 查询详情；审计日志接口为 `/api/audit-logs`；管理员接口为 `/api/admin/dashboard` 和 `/api/admin/tasks`；指标接口为 `/api/metrics/task-summary`。

**统一测试结论表述：**

系统测试应表述为：在本地开发与演示环境下，系统通过自动化测试、人工页面验证和真实异步联调，验证了用户认证、任务管理、SAST/DAST/SCA 扫描、报告导出、管理看板和审计日志等核心功能链路的可用性。测试结论不应扩展为生产级高并发能力，也不应将本地靶场扫描结果泛化为对所有公网目标的扫描能力。

**第 8 轮结论：**

经过本轮检查，当前论文草稿的章节结构、技术栈、功能模块和接口路径整体与项目实际实现保持一致。已直接修正角色大小写和返回值说明的歧义。剩余主要工作集中在第 6 章测试材料补充、图表编号定稿、参考文献引用标注补强和最终语言润色。正式定稿前，应重点替换所有 `【需补...】` 标记，并根据最终运行结果更新测试结论。
